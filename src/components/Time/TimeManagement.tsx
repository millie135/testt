...
// Helper function
const getLogDate = (log: TimeLog): Date | null => {
  return log.timestamp instanceof Timestamp ? log.timestamp.toDate() : null;
};
...
const TimeManagement = forwardRef<TimeManagementHandle, TimeManagementProps>(({ userId }, ref) => {
  ...
  if (logs.length > 0) {
    ...
    // Only logs for today with valid timestamps
    const dayLogs = logs.filter(
      (log) => log.timestamp instanceof Timestamp && log.timestamp.toDate().toDateString() === todayStr
    );
    ...
    dayLogs.forEach((log) => {
      ...
      //cast timestamp to Date explicitly
      const time = (log.timestamp as Timestamp).toDate() as Date;
    ...
    dailySummaries.push({
      date: todayStr,
      checkIn: checkInTime ? (checkInTime as Date).toLocaleTimeString() : "-",
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      const filtered = data.filter(log => getLogDate(log)?.toDateString() === today.toDateString());
      ...
      if (!filtered.length) setCurrentStatus("offline");
        ...
        switch (last.type) {
          case "checkin": setCurrentStatus("checkedIn"); break;
          case "breakStart": setCurrentStatus("onBreak"); break;
          case "breakEnd": setCurrentStatus("checkedIn"); break;
          case "checkout": setCurrentStatus("checkedOut"); break;
        }
      }
  ...
  const addLog = async (type: TimeLog["type"], breakType?: string, note?: string) => {
    ...
    switch (type) {
      case "checkin": setCurrentStatus("checkedIn"); updateStatus("online"); break;
      case "checkout": setCurrentStatus("checkedOut"); updateStatus("offline"); break;
      case "breakStart": setCurrentStatus("onBreak"); updateStatus("onBreak"); break;
      case "breakEnd": setCurrentStatus("checkedIn"); updateStatus("online"); break;
    }
  };
  ...
  return (
    <div className="flex flex-col space-y-4 w-full max-w-6xl mx-auto p-14 pt-8 pb-8 border-[1.5px] border-gray-200 rounded-md">
      ...
      >
        ...
        >
          {/* Date at top */}
          <div className="absolute top-0 px-3 py-1  text-white text-xl rounded-b-sm" style={{ backgroundColor: "#910A67" }}>
            ...
            <table className="min-w-full text-sm border-collapse border border-gray-300 dark:border-gray-600">
              <thead>
                <tr className="bg-gray-200 dark:bg-gray-700">
                  ...
                  <th className="border border-gray-300 dark:border-gray-600 px-2 py-1 text-left">Total Break</th>
                  <th className="border border-gray-300 dark:border-gray-600 px-2 py-1 text-left">Total Work</th>
